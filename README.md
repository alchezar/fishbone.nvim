...
| selection | blue   | lines in the active visual selection                   |
| viewport  | silver | lines currently visible on screen                      |

Where the selection and viewport bands overlap, the cell uses a 50/50 blend
of the two colors (the `selection_view` layer) so the intersection reads as
its own tone.
...
- `FbnT_<name>` - top-only cells (`cursor`, `search`, `mark`, `selection`,
  `selection_view`, `viewport`)
